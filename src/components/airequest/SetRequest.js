...
import styles from "./SetRequest.scss";
...
const CreateAIRequest = ({ user, updateRequestId }) => {
    ...
    const handleSet = async (value) => {
        ...
        try {
            const data = await fetch(`${cosmos.url}/provider/oscript/${oscriptName}`).then((res) =>
                res.json()
            );
    ...
    const handleClickSubmit = async () => {
        await trigger();
        if (errors && Object?.values?.(errors)?.length === 0) return onSubmit(getValues());
        else return;
    };
    ...
    return (
        ...
        <>
            <div className={cx("title")}>Set request</div>
            <FormProvider {...methods}>
                <div className={cx("set-request")}>
                    <form onSubmit={handleSubmit(onSubmit, onError)} className="keystation-form">
                        <FormInput
                            classNameCustom={`form-control, ${cx("account-input")}`}
                            wrapperStyle={cx("account")}
                            name="account"
                            errorobj={errors}
                        />
                        <FormInput
                        ...
                        <div className="keystation-tx-info" id="tx-info">
                            <div className={cx("row", "row-custom")}>
                                ...
                                <div class="col-4">{user?.address || "Test address....."} </div>
                            </div>
